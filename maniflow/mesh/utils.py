...
def coincidingVertices(mesh: Mesh):
    ...
    The approach is similar to a upper triangle matrix as we don't need to reverse check vertices,
    ...
    for i in range(mesh.v-1):
        ...
        for j in range(mesh.v-i-1):
            ...
            if i+1+j in lookup:
                continue
            if np.allclose(mesh.vertices[i], mesh.vertices[i+1+j], atol=1e-06):  # vertex coordiantes are same or at least very close
